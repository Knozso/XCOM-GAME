...
public class GrenadeMovingSinglePlayerScript : MonoBehaviour
{
        ...
        if (canMove)
        {
            ...
            if ((pos - end).magnitude < new Vector3(1f, 1f, 1f).magnitude)
            {
                ...
                for (int i = endCell.GetGridX() - 1; i <= endCell.GetGridX() + 1; i++)
                {
                    ...
                    {
                        if (i >= 0 && j >= 0 && i < GridSinglePlayer.grid.GetLength(0) && j < GridSinglePlayer.grid.GetLength(1))
                        {
                            Cell targetCell = GridSinglePlayer.grid[i, j];
