...
public class GridSinglePlayer : MonoBehaviour
{
    ...
    private void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                CellViewSinglePlayer cellView = currTileGameObject.GetComponent<CellViewSinglePlayer>();
                cellView.CreateCell();
    ...
    public static void ColorCellsAroundUnit(Unit unit)
    {
        if (unit != null && unit.Owner != null && !unit.Owner.GetType().Equals(typeof(AiPlayer)))
        {
            ...
            moveList = PathfindingSinglePlayer.GetCellsInRange(unit.CurrentCell, unit.WalkableDistance * 2);

            ResetCellsColor();

            foreach (Cell cell in moveList)
            {
                if (PathfindingSinglePlayer.GetPathDistance(unit.CurrentCell, cell) <= unit.WalkableDistance && unit.Actions >= 1 && cell.Walkable)
                {
                    cell.ChangeColor(UnityEngine.Color.yellow);
                    cell.Value += 50;
                }
                else if (PathfindingSinglePlayer.GetPathDistance(unit.CurrentCell, cell) <= unit.WalkableDistance * 2 && unit.Actions >= 2 && cell.Walkable)
                {
    ...
    public static void ColorCellsAroundUnitSpecific(Unit unit, UnityEngine.Color color, int distance)
    {
        ...
        {
            List<Cell> moveList = new List<Cell>();
            moveList = PathfindingSinglePlayer.GetCellsInSquareRange(unit.CurrentCell, distance);

            ResetCellsColor();

            foreach (Cell cell in moveList)
            {
                if (PathfindingSinglePlayer.GetDistance(unit.CurrentCell, cell) <= distance)
                {
    ...
    {
        ResetCellsValue();
        List<Cell> moveList = PathfindingSinglePlayer.GetCellsInSquareRange(unit.CurrentCell, unit.WalkableDistance * 2);
    ...
    public static void CalculateValuesForCellsBasedOnOtherUnits(Unit unit)
    {
        ...
        List<Cell> moveList = new List<Cell>();
        moveList = PathfindingSinglePlayer.GetCellsInSquareRange(unit.CurrentCell, unit.WalkableDistance * 2);
    ...
    public void AddEnemyPlayer()
    {
        ...
        {
            var player2 = new AiPlayer { Name = "Red", PlayerColor = Color.red };
        ...
        Stepper.Instance().Players.ForEach(p =>
        {
            if (p.PlayerColor.Equals(Color.blue))
            {
                foreach (Vector2Int cellCoord in playerUnitPositions)
                {
                    if (cellCoord.x >= 0 && cellCoord.x < gridSizeX && cellCoord.y >= 0 && cellCoord.y < gridSizeY)
                    {
                        ...
                        Unit unit = new Unit(false, false);
                        if(p.Units.Count==0)
                        {
                            unit = new Grenadier(false, false);
                        }
                        if(p.Units.Count==1)
                        {
                            unit = new Sniper(false, false);
                        }
                        if (p.Units.Count == 2)
                        {
                            unit = new Shotgunner(false, false);
                        }
                        unit.Owner = p;
                        p.Units.Add(unit);
            ...
            else
            {
                foreach (Vector2Int cellCoord in enemyUnitPositions)
                {
                    ...
                    {
                        Cell cellForUnit = grid[cellCoord.x, cellCoord.y];
                        Unit unit = new Unit(true, false);
                        if (GameObject.FindWithTag("PlayerVSAi") != null)
                        {
                            unit = new Unit(true, false);
                        }
                        else if (p.Units.Count == 0)
                        {
                            unit = new Grenadier(true, false);
                        }
                        else if (p.Units.Count == 1)
                        {
                            unit = new Sniper(true, false);
                        }
                        else if (p.Units.Count == 2)
                        {
                            unit = new Shotgunner(true, false);
                        }
                        unit.Owner = p;
                        p.Units.Add(unit);
                        //unit.PlaceUnit(cellForUnit);
    ...
    public void PlaceUnit(Unit unit, Cell cellForUnit)
    {
        ...
        System.String prefabString = unit.PrefabString;//unit.IsEnemyUnit ? "Prefabs/EnemyUnitSinglePlayer" : "Prefabs/PlayerUnitSinglePlayer";
        ...
        UnitViewSinglePlayer unitView = unitSprite.GetComponent<UnitViewSinglePlayer>();
        //unitView.InitUnit();
